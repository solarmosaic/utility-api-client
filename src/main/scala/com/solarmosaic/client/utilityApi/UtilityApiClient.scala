...
import com.solarmosaic.client.utilityApi.DataFormats.DataFormat
...
case class UtilityApiClient(
   ...
   */
  def response[T: FromResponseUnmarshaller](request: HttpRequest, format: DataFormat): Future[T] = connection
    .map(c => addHeader(Authorization(GenericHttpCredentials("Token", token)))
   ...
   */
  def getAccount(uid: String, format: DataFormat = DataFormats.json): Future[AccountResponse] =
